...
WebIDL::ExceptionOr<GC::Ref<Memory>> Memory::construct_impl(JS::Realm& realm, MemoryDescriptor& descriptor)
{
    ...
    auto memory_object = realm.create<Memory>(realm, *address, shared);
...
Memory::Memory(JS::Realm& realm, Wasm::MemoryAddress address, bool shared)
...
WebIDL::ExceptionOr<void> Memory::reset_the_memory_buffer()
{
    ...
    // FIXME: 1. Let map be the surrounding agent's associated Memory object cache.
    // FIXME: 2. Assert: map[memaddr] exists.
    // FIXME: 3. Let memory be map[memaddr].
    // FIXME: 4. Let buffer be memory.[[BufferObject]].
    ...
    if (m_buffer->is_fixed_length()) {
        ...
        // FIXME: 2. Let newBuffer be the result of creating a fixed length memory buffer from memaddr.
        // FIXME: 3. Set memory.[[BufferObject]] to newBuffer.
    }

    // 6. Otherwise,
    else {
        // FIXME: 1.  Let block be a Data Block which is identified with the underlying memory of memaddr.
        // FIXME: 2.  Set buffer.[[ArrayBufferData]] to block.
        // FIXME: 3.  Set buffer.[[ArrayBufferByteLength]] to the length of block.
    }

    // NOTE: the buffer should be created at 5.2, but current imlementation does not distinguish between fixed and resizable array buffers.
    m_buffer = TRY(create_a_fix_length_memory_buffer(vm, realm, m_address, m_shared));
...
WebIDL::ExceptionOr<GC::Ref<JS::ArrayBuffer>> Memory::create_a_fix_length_memory_buffer(JS::VM& vm, JS::Realm& realm, Wasm::MemoryAddress address, bool shared)
{
    auto& context = Detail::get_cache(realm);
    auto* memory = context.abstract_machine().store().get(address);
    if (!memory)
